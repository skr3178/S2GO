"""Phase 5 profile target — runs the tile-blocked kernel repeatedly so ncu
data concentrates on its forward + backward kernels.

Mirrors profile_target.py (which targets the Phase 3 oracle). The two .ncu-rep
files can then be diffed for the cache-hit-rate / memory-throughput / occupancy
before-after table.

Usage:
    /media/skr/storage/conda_envs/selfocc/bin/python profile_target_tiled.py
"""
import torch
import local_aggregate_s2go_tiled as op
...
def main(n_gauss: int = 12_800, n_iters: int = 30):
    ...
    # Tile-blocked kernel requires scanline-ordered pts
    vx = torch.arange(H, device=device)
    vy = torch.arange(W, device=device)
    vz = torch.arange(D, device=device)
    gx, gy, gz = torch.meshgrid(vx, vy, vz, indexing='ij')
    voxel_centers_int = torch.stack([gx, gy, gz], dim=-1).reshape(-1, 3).float()
    pmin_t = torch.tensor(pc_min, device=device)
    pts = ((voxel_centers_int + 0.5) * grid_size + pmin_t).unsqueeze(0).contiguous()

    g = torch.Generator(device=device).manual_seed(0)
    pmin = pmin_t
    pmax = pmin + torch.tensor([H, W, D], device=device).float() * grid_size
    means = ((torch.rand(1, n_gauss, 3, device=device, generator=g) * (pmax - pmin) * 0.95) + pmin).clone()
