	...
	const torch::Tensor& density_grad)
{
	...
	// Phase 4e: voxel2pts is unused by the tile-blocked backward, but keep
	// the placeholder allocation to satisfy the existing Aggregator::backward
	// signature (Phase 4e ignores the pointer).
	torch::Tensor voxel2pts = torch::full({1}, -1, means3D.options().dtype(torch::kInt32));
