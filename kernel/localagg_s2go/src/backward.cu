	...
	float* __restrict__ cov3D_grad)
{
	...
	for (int i = start; i < end; i++)
	{
		...
		if (pts_idx >= 0)
		{
			...
			// S2GO Eq. 9 backward: bin_logits gradient flows through alpha = power*opa
			//   dL/dpower (from bin) = bin_logits_grad * (1-bin_logits)/(1-alpha) * opa
			//   dL/dopa   (from bin) = bin_logits_grad * (1-bin_logits)/(1-alpha) * power  (NEW)
			float bin_alpha = power * opa;
			float bin_factor = (1 - bin_logits[pts_idx]) / (1 - bin_alpha + 1e-9) * bin_logits_grad[pts_idx];
			power_grad += bin_factor * opa;
